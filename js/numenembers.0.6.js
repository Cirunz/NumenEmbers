...
function showeffortpanel(sender){
    var $box = $('<div class="effort-panel" style="display: none;">' +
                ...
                '</div>' +
                '<div class="effort-panel-lower clearfix">' +
                '<div class="effort-panel-buttons effort-panel-leftbuttons">' +
                '<button type="button" class="effort-add button">+1</button>' +
                '<button type="button" class="effort-sub button">-1</button>' +
                '</div>' +
    ...
    $('.effort-add').unbind('click').click(function() {
        var selection = parseInt($box.find('.effort-value').val()) + 1;
        
        $box.find('.effort-value').val(selection);
        updateeffortpreview($box);
        
        return false; 
    });
    
    $('.effort-sub').unbind('click').click(function() {
...
function updateeffortpreview(effortbox){
    var selection = parseInt($(effortbox).find('.effort-value').val());
    var edge = parseInt($(effortbox).parent().find('.edge-value').val());
    
    if (selection > 1){
        enablebutton($(effortbox).find('.effort-sub'));
    } else {
        disablebutton($(effortbox).find('.effort-sub'));
    }
    
    var expense = calculate_effort(selection, edge);
    $(effortbox).find('.effort-preview').text(expense);
}
